...
--
function determineContributions(model, contribPath, model_l1)
    ...
    local inputHiddenWeights = layers[1].weight
    local inputDimen = inputHiddenWeights:size(2)
    local hiddenDimen = inputHiddenWeights:size(1)
        
    -- for each hidden node
    for h = 1, hiddenDimen do
    
        -- Denominator will be same for this hidden node
        local sum = 0
        for j = 1, inputDimen do
            sum = sum + math.pow(inputHiddenWeights[h][j], 2)
        end
        local denom = math.sqrt(sum)
        
        local hiddenContrib = torch.Tensor(inputDimen)
        for i = 1, inputDimen do
            local numer = inputHiddenWeights[h][i]
...
--
function saveOperation(model, model_l1, itrs, params, modelParams, wordDict, word2vec, dataset)
    ...
    if (contribsSaveItrs ~= nil and itrs % contribsSaveItrs == 0 and params.contribs == 'yes') then
        ...
        determineContributions(model, contribsFile, model_l1)
    end
...
--
--
function trainAutoencoder(model, model_l1, trainer, itrsCompleted, params, modelParams, dataset, clusters, word2vec, wordDict)
    ...
    saveOperation(model, model_l1, i, params, modelParams, wordDict, word2vec, dataset)
...
if (params.json ~= '') then
    ...
    local model, model_l1, layerMap = nil, nil, nil
    ...
    if (modelParams.data.model_l1 ~= nil) then
        if (string.find(modelParams.data.model_l1, ".txt") ~= nil) then
            ...
            -- model param is dir
            Logger:info("Loading model_l1 from file with greatest number in directory " .. modelParams.data.model)
            -- find most recent file
    ...
    if (params.contribs == 'yes') then
        if (modelParams.data.contribs ~= nil) then
            determineContributions(model, modelParams.data.contribs .. "default.txt")
        end
    end    
