...
if __name__ == "__main__":
    ...
    circle_mask = imread(os.path.join(d, "black_circle_mask_whitebg_300x300.png"))
        
    # assume each node word cloud is is 300 x 300
    wordCloudSize = (300, 300)
    padding = (20, 50)
    margins = (50, 50)
    ...
    while (c < numFilesL1):
        ...
        img.save(os.path.join(imageDir, str(c+1).zfill(znum) + '.jpg'), "JPEG")
    ...
    if (finalImgL1 == None):
        # load last img
        path = os.path.join(imageDir, str(numFilesL1).zfill(znum) + '.jpg')
